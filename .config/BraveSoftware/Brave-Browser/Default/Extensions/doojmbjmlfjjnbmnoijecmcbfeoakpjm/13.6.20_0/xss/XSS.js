  ...
  async function requestListener(request) {
    ...
    let prompting = (async () => {
      ...
      let {button, option} = await Prompts.prompt({
        ...
        options: [
          {label: _(`XSS_opt${block ? 'Block' : 'Sanitize'}`), checked: true}, 
          {label: _("XSS_optAlwaysBlock", [source, destOrigin])}, 
          {label: _("XSS_optAllow")}, 
          {label: _("XSS_optAlwaysAllow", [source, destOrigin])}, 
        ],
    ...
    async maybe(xssReq) { 
      ...
      return new Promise((resolve, reject) => {
        ...
        let ttlCheck = async () => {
          ...
          let now = Date.now();
          elapsed += (now - startTime) / workersCount; 
          if (elapsed < baseTTL) {
            startTime = now;
