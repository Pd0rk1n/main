...
complete -c curl -s s -l silent -d 'VERSION This man page describes curl 8. 15. 0'
...
complete -c curl -l doh-url -d 'Specify which DNS-over-HTTPS (DoH) server to use to resolve hostnames, instea…'
...
complete -c curl -l pass -d '(SSH TLS) Passphrase for the private key used for SSH or TLS'
...
complete -c curl -l suppress-connect-headers -d 'When -p, --proxytunnel is used and a CONNECT request is made, do not output p…'
...
complete -c curl -l upload-flags -d 'Specify additional behavior to apply to uploaded files'
