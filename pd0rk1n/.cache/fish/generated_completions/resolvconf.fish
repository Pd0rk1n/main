...
complete -c resolvconf -l raw -d 'Dump the answer records as binary data'
...
complete -c resolvconf -l relax-single-label -d 'Takes a boolean parameter; used in conjunction with query'
complete -c resolvconf -l no-ask-password -d 'Do not query the user for authentication for privileged operations'
complete -c resolvconf -l json -d 'Shows output formatted as JSON'
complete -c resolvconf -s j -d 'Equivalent to --json=pretty if running on a terminal, and --json=short otherw…'
