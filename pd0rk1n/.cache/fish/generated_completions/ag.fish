...
complete -c ag -l agrc -d 'Use FILE instead of $AGRC or $HOME/. agrc for default options'
...
complete -c ag -s C -l context -d 'Print lines before and after matches.  If not provided, LINES defaults to 2'
...
complete -c ag -s E -l extension -d 'Search files with this extension.  Equivalent to -j -G \''
...
complete -c ag -l help-types -d 'Alias for --list-file-types for compatibility with ack'
complete -c ag -l hidden -d 'Search hidden files.  This option obeys ignored files'
complete -c ag -s I -l ignore -d 'Ignore files/directories whose names match PATTERN'
...
complete -c ag -s j -l just-filename -d 'Search only the file name, not the full path, when using a file search regex …'
...
complete -c ag -s p -l path-to-ignore -d 'Provide PATH pointing to a specific . ignore file'
complete -c ag -s P -l pager -d 'Use a pager such as less.  Use --nopager to override'
...
complete -c ag -s q -l silent -d 'Suppress all log messages, including errors'
...
complete -c ag -s W -l width -d 'Truncate match lines after NUM characters'
complete -c ag -s X -l invert-file-search-regex -d 'Like -G, but only search files whose names do not match PATTERN'
complete -c ag -s z -l search-zip -d 'Search contents of compressed files.  Currently, gz and xz are supported'
complete -c ag -s Z -l null-lines -d 'Use NULL (0) as the newline separator instead of n'
