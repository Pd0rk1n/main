...
complete -c 'partclone.dd' -s W -l restore_raw_file -d 'create special raw file for loop device'
...
complete -c 'partclone.dd' -s i -l ignore_crc -d 'Ignore crc check error'
...
complete -c 'partclone.dd' -o aX -l checksum-mode -d 'Checksum formula to use to add error detection . sp where X: '
complete -c 'partclone.dd' -o kX -l blocks-per-checksum -d 'Write one checksum for every X blocks'
...
complete -c 'partclone.dd' -l binary-prefix -d 'Show progress with bit size (default is MB, GB. )'
complete -c 'partclone.dd' -l prog-second -d 'Show progress in seconds (default is minute)'
