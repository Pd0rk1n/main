...
complete -c ddrescue -s I -l check-input-size -d 'compare input file size with size in mapfile'
complete -c ddrescue -s J -l check-on-error -d 'reread latest good sector after every error'
...
complete -c ddrescue -l bad-sector-data -d 'treat sectors with <file> data as read errors'
complete -c ddrescue -l command-mode -d 'execute commands from standard input'
complete -c ddrescue -l continue-on-errno -d 'treat errno code <n> as non-fatal'
